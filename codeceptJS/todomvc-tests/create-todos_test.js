Feature('@first Create Todos @step:06 @smoke @story:12345 @Sd058d4f7')
 ...
 * Happy Path tests
 */
Scenario('Create a new todo item @Taef62638', async ({ I, TodosPage }) => {
...
Scenario('Create multiple todo items @Te82cfe60', async ({ I, TodosPage }) => {
  I.say('Given I have an empty todo list')
...
Data(examples).
Scenario('Todos containing weird characters @T077285b1', async ({ I, current, TodosPage }) => {
...
Scenario('Text input field should be cleared after each item @T672bdf9c', async ({ I, TodosPage }) => {
  I.say('Given I have an empty todo list')
...
Scenario('Text input should be trimmed @Tcdbdb4c4', async ({ I, TodosPage }) => {
  I.say('Given I have an empty todo list')
...
Scenario('New todos should be added to the bottom of the list @T8fd5a29b', async ({ I, TodosPage }) => {
...
Scenario('Footer should be visible when adding TODOs @T581f52f8', async ({ I, TodosPage }) => {
